...
# Read demand
df_demand = pd.read_excel(file, sheet_name="Data Demand")
demand = np.zeros((5, 100))
demand[0,:] = df_demand["Demand A"].T
demand[1,:] = df_demand["Demand B"].T
demand[2,:] = df_demand["Demand C"].T
demand[3,:] = df_demand["Demand D"].T
demand[4,:] = df_demand["Demand E"].T
demand = np.array(demand)

class TestComplexState(Generator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.edge_expected_lengths = np.full(self.num_edges, 2, dtype=self.int_dtype) # Randomise lead time
        ...
        for i in range(self.num_edges): # Random factor is set to negative for costs
            self.edge_costs[i, 0, :] = -cost_edge[i]
            self.edge_costs[i, 1, :] = -emission_edge[i]
        ...
        self.edge_costs[-self.num_market:, 0, :] = base_prices[:, np.newaxis]
        ...
        for i in range(self.num_nodes):
            self.node_costs[i, :, 0, :] = -cost_node[i]
            self.node_costs[i, :, 1, :] = -emission_node[i]
        ...
        self.edge_orders[-self.num_market:self.num_edges,:] = demand
