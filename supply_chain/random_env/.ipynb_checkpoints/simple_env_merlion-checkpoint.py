...
from state_generator.simple_state import SimpleState
...
class SimpleSC(gym.Env):
    ...
    def set_task(self):
        self.state = SimpleState(num_timesteps=100)()[0] #task
    ...
    def sample_tasks(self, n_tasks):
        ...
        return [SimpleState(num_timesteps=100)()[0] for _ in range(n_tasks)]           
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.state = SimpleState(num_timesteps = 100)()[0] #generate a new state
        ...
        self.vector_reward = np.array([0.0, 0.0, 0.0])
    ...
    def step(self, action):
        ...
        demand = self.state.edge_orders[-2:self.state.num_edges, self.timestep]
        
        sl = np.zeros(2)
        epsilon = 1e-12
        sl[0] = min((self.state.edge_inputs[4, self.timestep]
                      + self.state.edge_inputs[6, self.timestep])/(demand[0]+epsilon), 1)
        sl[1] = min((self.state.edge_inputs[5, self.timestep]
                      + self.state.edge_inputs[7, self.timestep])/(demand[1]+epsilon), 1)
