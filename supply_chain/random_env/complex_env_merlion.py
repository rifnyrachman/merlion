...
# max_profit = 5679.69
min_profit = 0
max_profit = 50_000 # default 50000
min_emission = 0
max_emission = 10_000
max_equity = 5
min_equity = 0
...
#============debugging===========
    ...
    #         i0 = tuple(idx[0])
    #         raise ValueError(
    #             f"{name} has non-finite at {i0}; value={x[i0]}; "
    #             f"nan_count={np.isnan(x).sum()} inf_count={np.isinf(x).sum()} "
...
#         return arr_obs_final
    ...
    def set_task(self):
        self.state = ComplexState(num_timesteps=100)()[0] #task
    ...
    def sample_tasks(self, n_tasks):
        ...
        return [ComplexState(num_timesteps=100)()[0] for _ in range(n_tasks)]
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.state = ComplexState(num_timesteps = 100)()[0] #generate a new state
# Defensive init
        if hasattr(self.state, "cost_counts"):
            self.state.cost_counts = np.nan_to_num(self.state.cost_counts, nan=0.0, posinf=0.0, neginf=0.0)
            self.state.cost_counts[:] = 0.0
