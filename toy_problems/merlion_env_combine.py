...
# Target set (you can toggle comments as needed)
# ENV_NAME = [
    # "mo-halfcheetah-v4",
...
#     # +1 means “maximize as-is”, -1 means “cost -> flip sign to treat as a benefit”
#     # Defaults chosen for common MO task conventions; override per your needs in config.
#     defaults: Dict[str, Sequence[int]] = {
#         "mo-halfcheetah-v4":           [1, -1],   # forward, control cost
#         "deep-sea-treasure-v0":        [1, -1],   # treasure, time
#         "resource-gathering-v0":       [1, -1],   # resources, enemy/time
#         "mo-mountaincar-v0":           [1, -1],   # progress, energy/time
#         "mo-mountaincarcontinuous-v0": [1, -1],   # progress, energy
#         "mo-lunar-lander-v2":          [1, -1],   # shaping, fuel
#         "water-reservoir-v0":          [1, -1],   # supply, flood risk
#     }
#     vec = defaults.get(env_id, [1] * d)
#     if len(vec) != d:
#         if len(vec) < d:
#             vec = list(vec) + [1] * (d - len(vec))
#         else:
#             vec = vec[:d]
...
class ScalarizedMetaEnv(gym.Env):
    ...
    def __init__(self, config: Dict):
        ...
        self.max_episode_steps = int(config.get("max_episode_steps", 1000))
            # self.max_episode_steps: int = max(int(config.get("max_episode_steps", 1000)), 1000)
            # Tidak set max_episode_steps, tidak pakai step cap
        # else:
        #     self.max_episode_steps = None
    
        # Untuk env lain, tetap pakai step cap dari config (atau default)
        # Build env first so we can infer reward dimension
    ...
    #     return [self._sample_task_obj() for _ in range(n_tasks)]
    ...
    def step(self, action):
        ...
        if isinstance(self.action_space, gym.spaces.Box):
            ...
            if act_noise_std > 0:
                ...
                # if self.action_space.bounded_above and self.action_space.bounded_below:
                #     action = np.clip(action, self.action_space.low, self.action_space.high)
        # print(self.env.map)
        ...
        # return np.asarray(obs, dtype=np.float32), self.scalar_reward, truncated_out, False, info
    ...
    def _apply_task(self, task: dict):
        ...
        if task is not None:
            for k, v in task.items():
                # Set di env (unwrapped) kalau atributnya ada
                if hasattr(un, k):
                    setattr(un, k, v)
                # Kalau ada di self (class), set juga (misalkan act_noise_std, dll)
                elif hasattr(self, k):
                    setattr(self, k, v)
            # Done, semuanya langsung inject dari dict
...
def randomize_parameter_env(env, env_name):
    ...
    if env_name == 'mo-halfcheetah-v4':
        ...
        # boleh kamu ubah kalau mau selalu True/False tetap
        ...
        # sesuai rekomendasi, gravity ≈ [-12, 0), wind_power [0, 20], turbulence [0, 2][web:106][web:108]
        ...
        # nr = 1 if size == 5 else np.random.choice([2, 4], 1)[0]
...
def reset_env1(env, env_name):
    ...
    if env_name == 'mo-halfcheetah-v4':
        ...
        g = params['gravity'] * rng(0.5, 1.5)
        g = float(np.clip(g, -12.0, -1e-3))   # batas gravity di LunarLander[web:127]
        w = params['wind_power'] * rng(0.5, 1.5)
        w = float(np.clip(w, 0.0, 20.0))      # batas wind_power[web:127]
        t = params['turbulence_power'] * rng(0.5, 1.5)
        t = float(np.clip(t, 0.0, 2.0))       # batas turbulence_power[web:127]
        env1.gravity          = g
        env1.wind_power       = w
...
def sample_four_room_maze(n1: int, n2: int, n3: int, seed: int = None):
    ...
    # kosongkan semua kecuali X
    for r in range(h):
        for c in range(w):
            if maze[r, c] != "X":
                maze[r, c] = " "

    # posisi pojok berseberangan kandidat start/goal
    def is_free(r, c):
        return maze[r, c] == " "
    ...
    # set start "_" dan goal "G"
    maze[sr, sc] = "_"
    maze[gr, gc] = "G"
    start_pos = (sr, sc)
    goal_pos = (gr, gc)

    # kumpulkan sel kosong untuk angka
...
def sample_treasures_from_map(noise_scale: float = 0.1, seed: int | None = None):
    ...
    # ambil semua cell dengan reward positif dan beri noise
    for i in range(h):
        for j in range(w):
            v = grid[i, j]
            if v > 0:
    ...
    # urutkan berdasarkan nilai treasure yang sudah di-noise
    treasures.sort(key=lambda x: x[2])

    # buat convex front: time objektif negatif terurut
    convex_front = []
