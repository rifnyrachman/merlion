...
class ScalarizedMetaEnv_notrandom(gym.Env):
    ...
    def set_weights(self, weights_dict: Dict[int, List[Union[List[float], np.ndarray]]]):
        # Store K weights for the given task id; does not auto-switch per step.
        # Your MIRACL driver should call set_weight(w_k) before collecting each subproblem rollout.
