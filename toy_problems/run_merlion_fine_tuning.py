...
def main():
    ...
    # parser.add_argument("--ppo-iters", type=int, default=100, help="PPO iterations per weight")
    # parser.add_argument("--num-workers", type=int, default=4, help="PPO rollout workers")
    # parser.add_argument("--frag-len", type=int, default=200, help="PPO rollout fragment length")
    # parser.add_argument("--max-episode-steps", type=int, default=100, help="Max episode steps passed via env_config")
    # parser.add_argument("--out-root", type=str, default="/mnt/hum01-home01/p88346bn/test/project/merlion/script/metamorl/mo_train_non_sc", help="Root directory for outputs/checkpoints")
    # parser.add_argument("--n-grid-2d", type=int, default=21, help="Number of weights on [0,1] grid for 2 objectives")
    # parser.add_argument("--n-sample-nd", type=int, default=21, help="Dirichlet samples for d_obj > 2")
    # parser.add_argument("--seed", type=int, default=0, help="Seed for Dirichlet sampler when d_obj > 2")
    ...
    #     runtime_env={
    #     "env_vars": {
    #         "PYTHONPATH": f"{MERLION_ROOT}:{MESSIAH_SRC}:" + os.environ.get("PYTHONPATH", "")
    #     }
    # }
    )
    ...
    ### record every = number of timestep per episode
    ### total_steps = total timesteps
    ### total_intervals = ppo_iters
